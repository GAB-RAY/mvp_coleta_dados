...
async function executar(){
  ...
  try{
    ...
    const payload=provider.montarPayload({telefone:'5521999999999',nomeContato:'Maria',templateNome:'template_qa_submeter',templateIdioma:'pt_BR',templateOrigem:'interno',templateStatusOficial:'APPROVED',templateComponentes:preparado.componentes,templateConfiguracaoEnvio:preparado.configuracaoEnvio});
    ...
    const payloadDuas=provider.montarPayload({telefone:'5521999999999',nomeContato:'Maria',bairroContato:'Centro',templateNome:'duas_variaveis_qa',templateIdioma:'pt_BR',templateOrigem:'interno',templateStatusOficial:'APPROVED',templateComponentes:duasVariaveis.componentes,templateConfiguracaoEnvio:duasVariaveis.configuracaoEnvio});
    ...
    await rejeitar(Promise.resolve().then(function(){return provider.montarPayload({telefone:'5521999999999',nomeContato:'Maria',bairroContato:null,templateNome:'duas_variaveis_qa',templateIdioma:'pt_BR',templateOrigem:'interno',templateStatusOficial:'APPROVED',templateComponentes:duasVariaveis.componentes,templateConfiguracaoEnvio:duasVariaveis.configuracaoEnvio});}),'nao possui a informacao necessaria para preencher {{2}}');
    ...
    const payloadImagemId=provider.montarPayload({telefone:'5521999999999',nomeContato:'Maria',templateNome:'imagem_id_qa',templateIdioma:'pt_BR',templateOrigem:'interno',templateStatusOficial:'APPROVED',templateComponentes:preparado.componentes,templateConfiguracaoEnvio:configuracaoImagemId});
    ...
    const payloadCta=provider.montarPayload({telefone:'5521999999999',templateNome:'cta_qa',templateIdioma:'pt_BR',templateOrigem:'interno',templateStatusOficial:'APPROVED',templateComponentes:cta.componentes,templateConfiguracaoEnvio:cta.configuracaoEnvio});
    ...
    const urlESair=templateService.prepararRascunho({nome:'URL e SAIR QA',categoria:'QA',metaNome:'url_e_sair_qa',metaIdioma:'pt_BR',metaCategoria:'MARKETING',ativo:true,
      componentes:[{type:'BODY',text:'Participe da nossa pesquisa.'},{type:'BUTTONS',buttons:[
        {type:'URL',text:'Quero participar',url:'https://example.com/pesquisa'},
        {type:'QUICK_REPLY',text:'SAIR'}
      ]}],configuracaoEnvio:{corpo:[],botoes:[{indice:1,subtipo:'quick_reply',origem:'opt_out'}]}});
    confirmar(urlESair.componentes[1].buttons[0].type==='URL'&&urlESair.componentes[1].buttons[1].type==='QUICK_REPLY','A ordem oficial URL + SAIR nao foi preservada.');
    const payloadUrlESair=provider.montarPayload({telefone:'5521999999999',templateNome:'url_e_sair_qa',templateIdioma:'pt_BR',templateOrigem:'interno',templateStatusOficial:'APPROVED',templateComponentes:urlESair.componentes,templateConfiguracaoEnvio:urlESair.configuracaoEnvio});
    confirmar(payloadUrlESair.template.components.length===1&&payloadUrlESair.template.components[0].index==='1'&&payloadUrlESair.template.components[0].parameters[0].payload==='nao_quero_mais_receber','SAIR nao usou o indice real no modelo misto.');
    await rejeitar(Promise.resolve().then(function(){return templateService.prepararRascunho({nome:'Botoes demais',categoria:'QA',metaNome:'botoes_demais_qa',metaIdioma:'pt_BR',metaCategoria:'MARKETING',ativo:true,componentes:[{type:'BODY',text:'Teste'},{type:'BUTTONS',buttons:[{type:'URL',text:'A',url:'https://a.example'},{type:'URL',text:'B',url:'https://b.example'},{type:'QUICK_REPLY',text:'SAIR'},{type:'QUICK_REPLY',text:'Outro'}]}],configuracaoEnvio:{corpo:[],botoes:[]}});}), 'um a tres botoes');
    await rejeitar(Promise.resolve().then(function(){return templateService.prepararRascunho({nome:'Dois SAIR',categoria:'QA',metaNome:'dois_sair_qa',metaIdioma:'pt_BR',metaCategoria:'MARKETING',ativo:true,componentes:[{type:'BODY',text:'Teste'},{type:'BUTTONS',buttons:[{type:'QUICK_REPLY',text:'SAIR'},{type:'QUICK_REPLY',text:'PARAR'}]}],configuracaoEnvio:{corpo:[],botoes:[{indice:0,subtipo:'quick_reply',origem:'opt_out'},{indice:1,subtipo:'quick_reply',origem:'opt_out'}]}});}), 'somente um botao');
    await rejeitar(Promise.resolve().then(function(){return provider.montarPayload({telefone:'5521999999999',templateNome:'sem_configuracao',templateIdioma:'pt_BR',templateOrigem:'interno',templateStatusOficial:'APPROVED',templateComponentes:[{type:'BODY',text:'Ola'},{type:'BUTTONS',buttons:[{type:'QUICK_REPLY',text:'Confirmar'}]}],templateConfiguracaoEnvio:{corpo:[],botoes:[]}});}), 'Configure o botão SAIR');
    await rejeitar(Promise.resolve().then(function(){return provider.montarPayload({telefone:'5521999999999',templateNome:'sem_imagem',templateIdioma:'pt_BR',templateOrigem:'interno',templateStatusOficial:'APPROVED',templateComponentes:[{type:'HEADER',format:'IMAGE'},{type:'BODY',text:'Ola'}],templateConfiguracaoEnvio:{corpo:[]}});}), 'Configure a imagem do cabeçalho');
