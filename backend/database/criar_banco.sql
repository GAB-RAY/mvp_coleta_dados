...
-- Para bancos existentes, execute somente as migrations pendentes.
...
BEGIN
  ...
  IF objeto_existente IS NOT NULL THEN
    RAISE EXCEPTION
      'Execução recusada: o banco já possui a estrutura do projeto (%). Use somente as migrations pendentes.',
      objeto_existente;
  END IF;
...
CREATE TABLE public.contatos (
  ...
  exclusao_solicitada_por_usuario_id BIGINT,
  ...
  CONSTRAINT contatos_solicitacao_exclusao_coerente CHECK (
    (exclusao_solicitada_em IS NULL AND exclusao_solicitada_por_usuario_id IS NULL)
    OR
    (exclusao_solicitada_em IS NOT NULL AND exclusao_solicitada_por_usuario_id IS NOT NULL)
  ),
...
AS $$
  SELECT EXISTS (
    ...
    WHERE contato.id = contato_id_consultado
      ...
      AND contato.excluido_logicamente = FALSE
      AND contato.exclusao_solicitada_em IS NULL
      AND EXISTS (
        SELECT 1
...
CREATE INDEX contatos_exclusao_solicitada_em_indice
  ON public.contatos (exclusao_solicitada_em DESC)
...
ALTER TABLE public.contatos
  ...
  ADD CONSTRAINT contatos_exclusao_solicitada_por_usuario_id_fkey
    FOREIGN KEY (exclusao_solicitada_por_usuario_id) REFERENCES public.usuarios(id);

ALTER TABLE public.consentimentos
  ADD CONSTRAINT consentimentos_contato_id_fkey
    FOREIGN KEY (contato_id) REFERENCES public.contatos(id),
...
VALUES
  ('Cadastro legado', 'cadastro-legado', 'legado', TRUE),
