...
function PreviaModeloMensagem({template}){
  ...
  const textoBotao=template.botaoTexto||(template.botaoTipo==='optout'?'Não quero mais receber':'Ação da mensagem');
  const botoesPrevia=Array.isArray(template.botoesOficiais)&&template.botoesOficiais.length
    ?template.botoesOficiais
    :(template.botaoTipo!=='nenhum'?[{indice:0,texto:textoBotao}]:[]);
...
function CampanhasAdministrativas(){
  ...
  const podeEnviar=Boolean(selecionada&&['pronta','ativa'].includes(selecionada.status)&&selecionada.modelo_meta_status_oficial==='APPROVED'&&quantidadeValida&&!enviandoCampanha);
  ...
  return <main className="pagina-administrativa"><div className="conteudo-administrativo campanhas-pagina">
    ...
    <section className="cartao campanhas-listagem">
      ...
      {carregando?<Carregando mensagem="Carregando campanhas..."/>:campanhas.length===0?<div className="estado-vazio-campanha"><strong>Nenhuma campanha cadastrada.</strong><span>Crie a primeira campanha para começar.</span></div>:<div className="grade-campanhas">{campanhas.map(function(item){return <article className={'cartao-campanha-resumo '+(selecionada&&selecionada.id===item.id?'ativo':'')} key={item.id}>
        ...
        <div className="rodape-cartao-campanha"><span>{formatarQuantidade(Number(item.enviado||0)+Number(item.entregue||0)+Number(item.lido||0))} enviados</span><button className="botao botao-secundario" type="button" onClick={function(){abrirCampanha(item);}}>Abrir campanha</button></div>
      </article>;})}</div>}
    </section>
    ...
    {selecionada&&<section className="cartao campanha-detalhes">
      ...
      {['rascunho','pronta','ativa'].includes(selecionada.status)&&<div className="bloco-envio-campanha">
    ...
    {administrador&&<details className="cartao secao-secundaria-campanha gerenciar-templates-campanha"><summary>Gerenciar modelos de mensagem</summary><div className="conteudo-templates-campanha">
      ...
      <div className="editor-template-campanha"><form onSubmit={salvarTemplate}>{!templateOficialEdicao&&<fieldset className="grade-criacao-campanha">
        ...
        <label className="campo-template-conteudo">Texto principal<textarea className="campo-input" value={template.conteudo} disabled={templateOficialEdicao} onChange={alterarTextoTemplate} placeholder="Ex.: Olá, {{1}}! Temos um convite para você." required/><small>{templateOficialEdicao?'Os valores entre chaves foram definidos no modelo oficial da Meta. Escolha abaixo de onde vem cada informação.':<>Valores como {'{{1}}'} e {'{{2}}'} são informações que mudam automaticamente em cada mensagem.</>}</small></label>
      ...
      </fieldset>}{templateOficialEdicao&&<fieldset className="configuracao-modelo-oficial">
        <section className="resumo-modelo-oficial"><span>Modelo oficial</span><strong>{template.nome}</strong><p>Aprovado pela Meta ✓</p></section>
